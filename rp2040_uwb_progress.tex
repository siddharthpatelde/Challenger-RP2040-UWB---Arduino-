...
\footnote{DW3000 User Manual (Decawave/Qorvo), section ``Single-sided two-way ranging'':
	...
	\begin{figure}[H]
		...
		\includegraphics[width=0.75\linewidth]{figures/ss_twr_timing.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{figures/dw_4msg.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{figures/dw_3msg.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\linewidth]{figures/TdoA.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{figures/2d_geo.pdf}
...
\begin{tcolorbox}[title=Note (project status: Sensor web --- data logging setup),
	colback=accentlight!60!white,
	sharp corners]
	...
	\textbf{Step 1: Configure each anchor (IDs + collision avoidance + target rate).}
	...
	\textbf{Step 2: Log the data on the PC.}
	...
	\(\{10, 30, 60, 100, 200, 500, 555\}\,\text{Hz}\),
...
(\(N=10000\) samples), including potential jitter, dropouts, and drift effects.
	...
	\begin{figure}[H]
		...
		\begin{minipage}{0.32\linewidth}
			\centering
			\includegraphics[width=\linewidth]{data_logger/sensor_web/CSVs/plots/2m_10.0Hz.png}
			
			\vspace{0.25em}
			{\footnotesize \texttt{2m\_10.0Hz.png}}
		\end{minipage}
		\hfill
		\begin{minipage}{0.32\linewidth}
			\centering
	...
	% ----------------------------------------------------
	\subsection{Throughput saturation and accuracy summary (2 m, long-run logs)}
	\label{subsec:2d-localization-accuracy-summary}
	% ----------------------------------------------------
	
	To summarize the \SI{2}{m} long-run experiment (\texttt{N=10000}), the CSV logs were processed with
	\texttt{analyze\_data.py} and the plots were generated with \texttt{web\_plot.py}. The saturation
	behaviour (target vs.\ actually achieved update rate) is shown in Figure~\ref{fig:freq-saturation-2m}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\linewidth]{data_logger/sensor_web/CSVs/plots/Frequency_Saturation_LogScale.png}
		\caption{Throughput saturation for the \SI{2}{m} test: configured target frequency vs.\ measured effective frequency (log scale).}
		\label{fig:freq-saturation-2m}
	\end{figure}
	
	\medskip
	\noindent
	Table~\ref{tab:uwb-2m-longrun-summary} lists the computed statistics for each configured rate.
	Here, the \emph{error} is defined as $\bar{d} - d_{\text{target}}$ (negative values indicate an underestimate).
	...
	\begin{table}[H]
		...
		\begin{tabular}{l
			...
			\textbf{N} \\
			\midrule
			\texttt{2m\_10.0Hz.csv}  & 10.0  & 9.18   & 108.94 & 1.8702 & -0.1298 & 0.0397 & 10000 \\
	...
	\noindent
	\textbf{Conclusion.}
	The system saturates at an effective maximum update rate of about \SI{260}{Hz} (at a target setting of \SI{555}{Hz}).
	However, the \SI{555}{Hz} run shows extremely unstable distance behaviour (very large standard deviation and a strong
	mean error), indicating that this operating point is not reliable. A practical compromise is to configure
	\SI{500}{Hz}, which achieves about \SI{227}{Hz} in practice while keeping the distance estimates stable.
